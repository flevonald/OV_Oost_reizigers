...
import os
import calendar
from folders import G01_folder, CHB_folder, CXX_HB_folder
#%% 
def O10_to_G01(df):    
...
g01_kolommen = ['DATAOWNERCODE','CONCESSIE','JAAR','MAAND','STOPPLACECODE'] + reizigerskolommen
...
#laad CHB gegevens
df_chb = pd.read_csv(os.path.join(CHB_folder,'CHB_transformed.csv'), sep=';',
...
for maandfolder in os.listdir(os.path.join(G01_folder,'KEOLIS')):
    ...
    for concessiefile in os.listdir(os.path.join(G01_folder,'KEOLIS',maandfolder)):
        if concessiefile.split('_')[0] in busconcessies:
            ...
            df_keolis = df_keolis.append(df_concessie, ignore_index=True)
...
for g01_file in os.listdir(os.path.join(G01_folder,'lelystad')):
    if g01_file.split('_')[1] == filter_jaar:
        ...
        df_arr_lls_file = df_arr_lls_file.rename(columns={'HNR':'STOPPLACECODE'})
        ...
        df_arr_lls = df_arr_lls.append(df_arr_lls_file)
...
koppeling_arriva = pd.read_excel(r"C:\data\O19\Arriva Koppeling.xlsx").set_index('code_lang')['stopplacecode'].to_dict()
...
df_arr_g01 = df_arr_g01.loc[~df_arr_g01['HALTE'].isin(['Mp Leonard','Kdl Europarcs','Kdl de Zandmeren'])]

  
#%% CXX

concessie_selectie = ['SAN','FL_IJM','OV_IJM']
...
for concessiefile in os.listdir(os.path.join(HB_folder,'CXX')):
    ...
    df = pd.read_csv(os.path.join(CXX_HB_folder,concessiefile), dtype=dtype_cxx,
    ...
    connexxion = connexxion.append(df)
...
df_totaal = df_totaal.append(df_keolis[g01_kolommen], ignore_index=True)
df_totaal = df_totaal.append(df_arr_g01[g01_kolommen], ignore_index=True)
df_totaal = df_totaal.append(df_arr_lls[g01_kolommen], ignore_index=True)
df_totaal = df_totaal.append(connexxion[g01_kolommen], ignore_index=True)
...
for concessie, df_tot_conc in df_totaal.groupby('CONCESSIE'):
    ...
    if 'YY' in df_tot_conc['MAAND'].to_list():
        df_halte_c = df_tot_conc.loc[df_group['MAAND']=='YY'].groupby(['STOPPLACECODE'])[reizigerskolommen].sum()
        # return df_group_o
    ...
    if 'XX' in df_tot_conc['MAAND'].to_list():
        ...
        df_group_x = df_group_x.groupby(['STOPPLACECODE','CONCESSIE'])[reizigerskolommen].sum()
    ...
    if maandset.issubset(set(df_tot_conc['MAAND'].to_list())):
        ...
        df_group_m = df_tot_conc.groupby(['STOPPLACECODE','CONCESSIE'])[reizigerskolommen].sum()
    ...
    #ander is er een deel van de maanden
        ...
        df_group_o = df_group_o.groupby(['STOPPLACECODE','CONCESSIE'])[reizigerskolommen].mean().fillna(0)
    ...
    df_per_halte_per_concessie = df_per_halte_per_concessie.append(df_group_o.reset_index())

#tel alle concessies bij elkaar op.
df_per_halte = df_per_halte_per_concessie.groupby('STOPPLACECODE')[reizigerskolommen].sum()

print('Reizigers per halte en per concessie bepaald')

#%%
#VOEG NAAM EN PLAATS VAN STOPPLACE TOE op basis van chb
df_per_halte_chb = df_per_halte.merge(df_chb[['STOPPLACECODE','NAME','TOWN']].drop_duplicates('STOPPLACECODE'), on=['STOPPLACECODE'], how='left')

df_per_halte_chb[['STOPPLACECODE','NAME','TOWN']+reizigerskolommen].to_csv(f'instappers per halte {filter_jaar}.csv',sep=';', decimal=',', index=False)
# voeg coördinaten toe (gemiddelde van quays per stopplace, afgerond op 1 decimaal (0.1m))
stopplace_coordinates = df_chb.groupby('STOPPLACECODE')[['RD-X','RD-Y']].mean().round(1).reset_index()
df_per_halte_coord = df_per_halte_chb.merge(stopplace_coordinates, on='STOPPLACECODE')
df_per_halte_coord.to_csv(f'instappers per halte {filter_jaar} met coordinaten.csv', sep=';', index=False)
